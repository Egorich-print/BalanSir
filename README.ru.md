...
| Смерть локального рантайма | L2 lifecycle watchdog (ADR-033): ограниченный guard перезапуска/восстановления |
...
endpoint'ов. Для pool-driven рантайма он владеет **L2 watchdog** (ADR-033).
...
**L2 watchdog (реализован)** — ограниченный guard перезапуска/восстановления во
владении `XrayManager` для *pool-driven* активного рантайма. Намеренно отделён от
L1 pool health-модели и никогда не влияет на ранжирование/выбор профилей:

- **Grace window** запуска (по умолчанию 10 с) терпит не-Healthy результаты,
  пока драйвер поднимается; после её окончания не-Healthy трактуется как улика.
- При падении вне grace перезапускается **тот же** драйвер (никогда не меняется
  VPN-профиль), с соблюдением **backoff**-паузы (по умолчанию 5 с).
- Перезапуски ограничены: **max_restarts** (по умолчанию 3) в скользящем окне
  **window_ms** (по умолчанию 60 с). Когда бюджет исчерпан, рантайм **останавливается
  (exhaust)** — трафик идёт напрямую; бесконечного цикла перезапуска нет. Новый
  старт даёт новый ограниченный бюджет.
- Покрыт юнит-тестами: grace, улика, ограниченное исчерпание бюджета, backoff-паузы,
  перезапуск того же драйвера, остановка рантайма при исчерпании, отсутствие
  ротации кандидатов.
...
Базовый `main`: **483 теста проходят, 0 падает, 4 игнорируются** (netns-тесты,
