...
7. **Абсолютные пути бинарников** (задача 1.6): внешние утилиты
   (`nft`, `ip`, `pkill`, `pgrep`, `ss`, `lsmod`, драйверные бинарники)
   резолвятся через `balansir_common::paths::resolve_bin`, который сначала
   ищет в стандартных каталогах (`/usr/bin`, `/usr/sbin`, `/sbin`, ...), и лишь
   затем через `$PATH`, чтобы привилегированные процессы не подхватывали
   исполняемые файлы из подконтрольного пользователю `$PATH`.

8. **API** (задача 1.7): `start_server` биндится на `127.0.0.1` (loopback)
   вместо `0.0.0.0`. Необязательная bearer-аутентификация включается через
